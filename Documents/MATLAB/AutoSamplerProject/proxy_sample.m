...
S = Slicer(Sample,Nslice,zeros(size(Sample)));              % Create instance of "Slicer" class using "Sample" as audio and "Nslice" as the number of slices to create.
...
step = 0.01;                                                  % Stepsize to be used in dirfference quotient for gradient descent. "step" must be whole number.
NumIterations = 200;
errorVec = zeros(1,NumIterations);
figure

for number = 1:NumIterations
    S.mkRender(S.getsliceCoeff);
    plot(S.getOutRender)                                      % Plot the vector generated by "S.mkRender" once it was called by "getError".
    title('Synthesized Audio')
    pause(0.1)
    gradient = G.grad(getError,step,S.getsliceCoeff);         % Calculate the gradient of the "getError" function at the point X0 = [abs(S.getsliceShift),S.getsliceCoeff]. ciel(abs(step)) ensures that the "step" value passed to G.grad is positive and a whole number.
    errorVec(number) = getError(S.getsliceCoeff);
    S.setsliceCoeff(S.getsliceCoeff-step*gradient);           % Descend gradient for "Coeff" values.
end
figure
plot(errorVec)
